...
def PROFILE(request):
    ...
    return render(request,'tec/profile.html',context)
...
def PROFILE_UPDATE(request):
    if request.method == "POST":
        ...
        try:
            ...
            return redirect('tec_profile')
        except:
            messages.error(request,"Your profile updation has been failed")
    return render(request, 'tec/profile.html')
...
def CHANGE_PASSWORD(request):
     ...
     if request.method == "POST":        
        ...
        if check == True:
          ...
          return redirect("tec_change_password")
     return render(request,'tec/change-password.html')